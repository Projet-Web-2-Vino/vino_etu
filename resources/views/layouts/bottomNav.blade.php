...
<div class="w-full fixed bottom-20">
  ...
  <nav class="fixed bottom-0 inset-x-0 bg-gray-100 flex justify-between text-sm text-blue-900 uppercase font-mono">
	...
	@if($titre == 'bouteille')
		@isset($id_cellier)
			<a href={{ route('bouteille.nouveau', ['id' => $id_cellier])}}  class="w-full block py-5 px-3 text-center">
				<i class="fa-solid fa-wine-bottle mb-2 mx-auto"></i>
				<span class="tab tab-explore block text-xs">Ajouter</span>
			</a>
			@endisset
	@endif
	...
	@if($titre == 'formBouteille')
				@isset($id_cellier)
					<a href={{ route('bouteille.liste', ['id' => $id_cellier])}}  class="w-full block py-5 px-3 text-center">
						<i class="fa-solid fa-wine-bottle mb-2 mx-auto"></i>
						<span class="tab tab-explore block text-xs">Liste</span>
					</a>
					@endisset
